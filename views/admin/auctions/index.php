	...
	<table border="0" class="table-list">
		<thead>
			<tr>
				...
				<th><?php echo lang('store:auctions:label:status'); ?></th>
		...
		<tbody>
			<?php foreach($auctions as $auction) { ?>
				<tr>
					<td><?php echo form_checkbox('action_to[]', $auction->id); ?></td>
					...
					<td><?php 
							...
							echo $output;  
					?></td>
<td><?php echo lang('store:auctions:label:status:'.$status_list[$auction->status]); ?></td>
					<td>
					<?php 
						$now = time();
					...
					<td class="align-center buttons buttons-small">
					   <?php $title = 'title="'. ucfirst($auction->category->name) . ' - ' . ucfirst($auction->name) . '" '; ?>
						...
						<?php echo anchor('admin/store/auctions/edit/' . $auction->id, lang('store:auctions:buttons:edit'), 'class="edit_auction btn orange"'); ?>
						<?php echo anchor('admin/store/auctions/delete/' . $auction->id, lang('store:auctions:buttons:delete'), array('class'=>'confirm btn red delete')); ?>
					</td>
				</tr>
			<?php } ?>
		</tbody>
